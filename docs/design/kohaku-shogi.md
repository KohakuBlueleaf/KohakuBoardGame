...
6. [Game Complexity](#6-game-complexity)
7. [AI/NNUE Feasibility](#7-ainnue-feasibility)
8. [Comparison to Existing Variants](#8-comparison-to-existing-variants)
9. [Fun Factor](#9-fun-factor)
...
| King (K) | a1 |
| Gold (G) | b1 |
| Silver (S) | c1 (back rank), b2 (defensive) |
| Lance (L) | d1 |
| Rook (R) | e1 |
| Bishop (B) | f1 |
| Knight (N) | a2 |
...
| King (k) | f7 |
| Gold (g) | e7 |
| Silver (s) | d7 (back rank), e6 (defensive) |
| Lance (l) | c7 |
| Rook (r) | b7 |
| Bishop (b) | a7 |
| Knight (n) | f6 |
...
corresponding piece at (6-file, 8-rank). For example:

- Sente King at a1 <-> Gote King at f7
- Sente Gold at b1 <-> Gote Gold at e7
- Sente advanced Pawn at a3 <-> Gote advanced Pawn at f5
- Sente Knight at a2 <-> Gote Knight at f6
...
### 3.4 Win Condition and Game End
...
e-f) houses the major pieces (Rook and Bishop) ready for offense. This
King-side vs. attack-side dichotomy mirrors the structure of real shogi
positions post-castling.

### 4.3 Why Gold Adjacent to King?

The Gold General sits at b1, immediately adjacent to the King at a1. This
mirrors the most fundamental defensive principle in shogi: the Gold belongs
next to the King. In every major castle formation in standard shogi (Yagura,
Mino, Anaguma, Bear-in-the-Hole), at least one Gold is directly beside the
King.
...
Kohaku Shogi includes **two Silver Generals** per side --- one on the back rank
(c1) and one on the second rank (b2). This dual-Silver design serves two
distinct roles:
...
2. **Offensive Silver (c1)**: Starting on the back rank behind the pawn wall,
...
The Knight is placed at a2 (Sente) / f6 (Gote), on the second rank adjacent
to the King-side. This placement serves dual purpose: it contributes to the
King's defensive shell while also being ready to jump forward (to b4 or a4
on its first move from a2, after advancing) for tactical operations.
...
### 4.7 Why 180-Degree Rotational Symmetry?
...
### 4.8 Why ALL Standard Piece Types?
...
As detailed in Section 4.7, the position has perfect 180-degree rotational
...
**Expected first-mover advantage**: Sente wins ~52-54% of games. This is
within the acceptable range for a competitive variant. For comparison:
...
### 5.4 Expected Game Length

**Estimated average game length**: 50-80 moves (100-160 ply).

Factors pushing the game longer:
- Dense pawn wall requires multiple exchanges to open.
- Pre-castled King is well-defended from the start.
- 14 pieces per side = more material to work through.

Factors pushing the game shorter:
- Compact board means pieces reach the enemy camp quickly once files open.
- Knight drops can create sudden tactical crises.
- 42 squares means less room to flee --- Kings run out of escape squares faster.
...
### 6.5 Comparison to Other Variants
...
## 7. AI/NNUE Feasibility

### 7.1 HalfKP Feature Size
...
### 7.2 Why NNUE Matters for This Variant
...
### 7.3 Datagen Considerations

**Search depth for datagen**: Depth 6 should be sufficient.
...
- **Iterative improvement**: Start with 5M positions from random self-play,
  ...
  search for the next iteration. 3-5 iterations typically converge to strong
  play.

---

## 8. Comparison to Existing Variants
...
| **Game length (ply)** | ~40 | ~45 | ~70 | **~65** | ~115 |
...
diversity and hand type diversity are preserved at 100%. This means Kohaku
Shogi sacrifices scale but not vocabulary --- the same strategic language
applies, spoken in shorter sentences.
...
### 9.1 What Makes Kohaku Shogi Fun to PLAY

**Immediate action**: The pre-castled start means no tedious setup phase. From
move 1, every move is a real strategic decision: push a pawn to probe? Swing
the Rook to a different file? Advance a Silver for attack? The game starts at
...
opens a lane for the Rook but also exposes the King-side. Pushing the f-file
Pawn activates the Bishop's diagonal but weakens the attack wing. Every pawn
...
**Quick but not shallow**: Expected game length of 50-80 moves means a game
takes 5-15 minutes at human speed. Long enough for real strategy, short enough
to play many games in a session. This makes it excellent for casual play,
tournament events, and especially for AI training where millions of games must
be played.

### 9.2 What Makes Kohaku Shogi Interesting for AI RESEARCH
...
**Opening theory exploration**: The pre-castled, symmetric start position
creates a well-defined opening theory problem. Which pawn breaks are optimal?
Should the Rook swing to the opposite wing (furibisha-style) or stay on its
home file (ibisha-style)? Does the Knight develop early or stay back for
defense? These questions have answers that NNUE-guided search can discover,
and the answers should be nontrivial.

**Benchmark for search algorithms**: At branching factor ~35 and game length
~65 ply, Kohaku Shogi occupies the sweet spot for comparing search algorithms
(alpha-beta, MCTS, hybrid approaches). It is complex enough that naive search
fails but tractable enough that algorithmic improvements produce measurable
Elo gains.
...
### 9.4 Opening Theory Speculation

The initial position suggests several natural opening strategies:

**Ibisha (Static Rook)**: Keep the Rook on e1 and open the e-file by
exchanging the e2 Pawn. The Rook controls the central file, and the Bishop
at f1 activates on the long diagonal once the f2 Pawn advances.

**Furibisha (Ranging Rook)**: Swing the Rook from e1 to c1 or d1, behind a
central pawn push. The Rook supports a c-file or d-file break while the
King-side remains fortified behind the advanced a3/b3 Pawns.

**Early Knight development**: From a2, the Knight can jump to b4 (if the b3
Pawn advances first) or directly participate in a King-side defense. An early
Knight jump creates an outpost that threatens enemy Pawns and pieces.

**Silver charge**: The offensive Silver at c1 can advance through d2 (after the
Pawn moves) to e3 and beyond, supporting a pawn push on the center or
attack wing. The classic "climbing Silver" (bougin) strategy from 9x9 shogi
translates directly.

**Bishop exchange line**: If both Bishops can be exchanged early (via diagonal
opening), the resulting Bishop-in-hand creates powerful drop threats. A Bishop
dropped behind enemy lines can threaten the King directly. The decision of
whether to exchange Bishops or maintain the Bishop on the board is a
fundamental strategic choice.

These opening themes mirror real shogi theory in compressed form. The
pre-castled position means the opening phase focuses entirely on piece
activation and pawn structure rather than King safety --- exactly the kind of
strategic content that rewards deep understanding.
