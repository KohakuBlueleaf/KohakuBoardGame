"""Standard Shogi (9x9) game module for CLI -- board display, human input, game logic."""
...
try:
    ...
    raise ImportError(
        "Shogi CLI requires gui.games.shogi_engine. "
        "Make sure the gui package is on sys.path."
    )

COL_LABELS = "abcdefghi"
ROW_LABELS = "987654321"  # row_labels[internal_row] = display rank
...
_HAND_NAMES = {1: "P", 2: "S", 3: "G", 4: "L", 5: "N", 6: "B", 7: "R"}
...
def uci_to_move(uci_str):
    ...
    Formats:
        ...
        'a1b2+'  -> ((from_r, from_c), (to_r + 9, to_c))    promotion
        'P*c3'   -> ((9, piece_type), (to_r, to_c))          drop
    """
...
def move_to_uci(move):
    ...
    Promotion move:   ((fr,fc),(tr+9,tc))           -> 'a1b2+'
    Drop move:        ((9, piece_type),(tr,tc))     -> 'P*c3'
    """
    (fr, fc), (tr, tc) = move
...
def print_board(state, game_ctx):
    """Print Standard Shogi (9x9) board with piece abbreviations and hand pieces."""
    print()
...
def get_human_move(state, game_ctx):
    """Prompt human player for a Shogi move via numbered list or algebraic notation."""
...
def get_context():
    """Return the game context dict for Standard Shogi (9x9)."""
    ctx = {
        "name": "shogi",
        "state_class": ShogiState,
