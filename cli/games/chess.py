"""Standard Chess game module for CLI -- board display, human input, game logic."""
...
try:
    ...
    raise ImportError(
        "Chess CLI requires gui.games.chess_engine. "
...
def print_board(state, game_ctx):
    """Print standard 8x8 Chess board with Unicode chess pieces from White's perspective."""
    print()
...
def get_human_move(state, game_ctx):
    """Prompt human player for a Chess move via numbered list or algebraic notation."""
    ...
    while True:
        try:
            raw = input(f"  Enter move number (or algebraic e.g. e2e4): ").strip()
        ...
        print(
            f"  Could not parse '{raw}'. Enter a move number or algebraic (e.g. e2e4)."
        )
...
def uci_to_move(uci_str):
    ...
    Formats:
        'e2e4'      -> ((from_r, from_c), (to_r, to_c))    board move
...
def get_context():
    """Return the game context dict for standard Chess."""
    ctx = {
        "name": "chess",
        "state_class": ChessState,
